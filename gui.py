...
class MedvisionApp:
    ...
    def _build_layout(self):
        ...
        # --- Asset checker (3 required templates + 1 optional tab fallback) ---
        assets_frame = ttk.Frame(self.root, style="Panel.TFrame")
        assets_frame.pack(fill="x", padx=pad, pady=5)
        ttk.Label(
            assets_frame, text="Template images (required, except the tab one is optional):",
            style="Panel.TLabel"
        ).pack(anchor="w", padx=10, pady=(8, 0))
    ...
    def _on_ready_to_sign(self, record):
        def show_popup():
            ...
            ttk.Label(
                win,
                text=(
                    f"Expected Patient ID: {record.patient_id}\n\n"
                    "IMPORTANT: Check the patient name/ID shown at the top of\n"
                    "the note on screen actually matches the ID above before\n"
                    "continuing - a slow-loading record can occasionally leave\n"
                    "the previous patient's note open by mistake.\n\n"
                    "Click 'Continue' only after verifying, to let the tool\n"
                    "type the password and sign - or close this and abort if\n"
